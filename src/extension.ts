...
let myStatusBerItem: vscode.StatusBarItem;
...
export function activate(context: vscode.ExtensionContext) {
	myStatusBerItem = vscode.window.createStatusBarItem(
		vscode.StatusBarAlignment.Left, 10000
	);

	context.subscriptions.push(myStatusBerItem);
...
function updateStatusBar(context: vscode.ExtensionContext): void {
	...
	myStatusBerItem.text = `${icon} ${name}`;
	myStatusBerItem.tooltip = tooltip;
	myStatusBerItem.show();
